...
def merge(data):
    ...
    for row in cursor:
        ...
        PassportIssueDate=row[11]
        PassportExireDate=row[12]
        PlaceOfBirth=row[13]
        # CompanyId=row[14]
        CreateDt=row[15]
        Occupation=row[16]
        ...
        newCustomer['createDt'] = CreateDt
        newCustomer['passIssueDate'] = PassportIssueDate
        # newCustomer['mahramName'] = 
        ...
        result.update({hajId: newCustomer})
        # Add packageName
        return result
    return result
