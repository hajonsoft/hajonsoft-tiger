...
const hijraMonths = [
  ...
  const eventsNearby = (inputDate) => {
    ...
    return `${output[0].feast.names.join(',')} ${output[0].daysBefore} days ago while ${output[1].feast.names.join(',')} in ${output[1].daysAfter} days `;
  }

  const todayHijraDate = ()=> {
    return `${imoment().iDate()}-${hijraMonths[imoment().iMonth()]}-${imoment().iYear()}`
  }
  export {eventsNearby, todayHijraDate}